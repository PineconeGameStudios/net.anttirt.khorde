...
namespace Khorde.Entities
{
    ...
    /// <remarks>
    /// UntypedBufferLookup is a native container that provides array-like access to DynamicBuffer components of a specific
    /// type. For example, while iterating over a set of entities, you can use UntypedBufferLookup to get and set  DynamicBuffers of unrelated entities.
    ///
    /// To get a UntypedBufferLookup, call <see cref="SystemState.GetUntypedBufferLookup"/>.
    ///
    /// Pass a UntypedBufferLookup container to a job by defining a public field of the appropriate type
    /// in your IJob implementation. You can safely read from UntypedBufferLookup in any job, but by
    ...
    /// by adding [NativeDisableParallelForRestrictionAttribute] to the UntypedBufferLookup field definition in the job struct.
    ///
    ///
    ...
    public unsafe struct UntypedBufferLookup
    {
                                  ...
                                  int bufferCapacity, int elementSize, int alignOf)
        {
            ...
            m_InternalCapacity = bufferCapacity;
        }
        ...
        public bool TryGetBuffer(Entity entity, out UntypedDynamicBuffer bufferData, out bool entityExists)
        {
            ...
            {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                bufferData =  new UntypedDynamicBuffer(header, m_Safety0, m_ArrayInvalidationSafety, m_IsReadOnly != 0, false, 0, m_InternalCapacity, m_ElementSize, m_AlignOf);
#else
                bufferData = new UntypedDynamicBuffer(header, m_InternalCapacity, m_ElementSize, m_AlignOf);
#endif
                return true;
            }
            else
        ...
        /// <remarks>
        /// Normally, you cannot write to buffers accessed using a UntypedBufferLookup instance
        ...
        /// by putting the [NativeDisableParallelForRestriction] attribute on the UntypedBufferLookup field in the Job.
        ///
        /// [NativeDisableParallelForRestrictionAttribute]: https://docs.unity3d.com/ScriptReference/Unity.Collections.NativeDisableParallelForRestrictionAttribute.html
        /// </remarks>
...
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                return new UntypedDynamicBuffer(header, m_Safety0, m_ArrayInvalidationSafety, m_IsReadOnly != 0, false, 0, m_InternalCapacity, m_ElementSize, m_AlignOf);
#else
        ...
        /// <summary>
        /// When a UntypedBufferLookup is cached by a system across multiple system updates, calling this function
