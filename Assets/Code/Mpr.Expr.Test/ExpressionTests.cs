...
public unsafe class ExpressionTests : ExpressionTestBase
{
	...
	public void Test_Variable()
	{
		...
		var asset = ScriptableObject.CreateInstance<ExpressionAsset>();
		try
		{
			asset.SetAssetData(baker.Builder, BlobExpressionData.SchemaVersion);
			...
			Assert.AreEqual("TestVar", layoutVariables[0].name);
			Assert.AreEqual(4, layoutVariables[0].length);
			Assert.AreEqual(0, layoutVariables[0].offset);
			Assert.AreEqual(true, layoutVariables[0].isGlobal);
			...
			blackboardBytes.ReinterpretStore(0, 3.54f);

			var ctx = new ExpressionEvalContext(ref blob, default, default, blackboardBytes, ref blackboardLayout);

			Assert.AreEqual(3.54f, n11.Evaluate<float>(in ctx));
		}
		finally
		{
